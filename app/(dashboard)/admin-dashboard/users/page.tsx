...
// import ViewUserModal from "@/components/admin-dashboard/modals/ViewUserModal";
// import EditUserModal from "@/components/admin-dashboard/modals/EditUserModal";
...
export default function AdminUsersPage() {
  const { data, isLoading, refetch } = useFetch<{ data: IUser[] }>(
    "/users"
  );
  ...
  const handleMakeAdmin = async (user: IUser) => {
    Swal.fire({
      title: "Are you sure?",
      text: `Make "${user.name}" an admin? They will have full access to the admin panel.`,
      icon: "warning",
  ...
  const handleRemoveAdmin = async (user: IUser) => {
    Swal.fire({
      title: "Are you sure?",
      text: `Remove admin privileges from "${user.name}"?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#232156",
      cancelButtonColor: "#d33",
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          Swal.fire({
            title: "Success!",
            text: `Admin privileges removed from ${user.name}.`,
            icon: "success",
            timer: 2000,
            showConfirmButton: false,
          });
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          <Button
            ...
            disabled={isDeleting || isMakingAdmin}
          >
