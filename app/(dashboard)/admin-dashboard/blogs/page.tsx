...
import CreateBlogModal from "@/components/admin-dashboard/modals/CreateBlogModal";
import EditBlogModal from "@/components/admin-dashboard/modals/EditBlogModal";
import ViewBlogModal from "@/components/admin-dashboard/modals/ViewBlogModal";
...
export default function AdminBlogsPage() {
  const { data, isLoading, refetch } = useFetch<{ data: IBlog[] }>(
    "/blogs",
  );
  ...
  const columns: ColumnDef<IBlog>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0 w-16 h-12 rounded-lg bg-primary/10 flex items-center justify-center overflow-hidden relative">
            {row.original.image ? (
              <Image
