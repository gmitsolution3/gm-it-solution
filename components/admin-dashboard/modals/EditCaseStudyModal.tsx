...
export default function EditCaseStudyModal({
  isModalOpen,
  setIsModalOpen,
  caseStudyData,
  onSuccess,
}: EditCaseStudyModalProps) {
  const { mutate: updateData, isLoading } = usePatch(
    `/case-studies`,
    {
  ...
  const { data: portfoliosData, isLoading: isLoadingPortfolios } =
    useFetch<{ data: IPortfolioItem[] }>("/portfolios");
    ...
    <Dialog open={isModalOpen} onOpenChange={handleClose}>
      <DialogContent className="!max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        >
          ...
          <FieldSet>
            <Field>
              <FieldLabel>Portfolio Item</FieldLabel>
              <FieldContent>
                <Select
                  onValueChange={(value) =>
                    form.setValue("portfolioId", value)
                  }
                  value={form.watch("portfolioId")}
              ...
              <FieldError>
                {form.formState.errors.portfolioId?.message}
              </FieldError>
            </Field>
          </FieldSet>
